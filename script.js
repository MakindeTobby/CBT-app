...
function showQuestions(index) {
    ...
    for (let i = 0; i < questions[index].options.length; i++) {
        ...
        if (sel_ans[que_count] == opt) {
            option_list.innerHTML += `
        <label for="optio${i + 1}"><div class="option"><input type="radio" checked name="options" id="optio${i + 1}" onchange="selectedAnswer('${opt}')">
        ${opt}</div></label>
`
        } else {
            option_list.innerHTML += `
    
        <label for="optio${i + 1}"><div class="option"><input type="radio"  name="options" id="optio${i + 1}"  onchange="selectedAnswer('${opt}')">
        ${opt}</div></label>
